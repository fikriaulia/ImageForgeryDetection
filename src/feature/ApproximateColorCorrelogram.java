...
public class ApproximateColorCorrelogram implements Feature{
    ...
    public ApproximateColorCorrelogram() {
        ...
        for (int di = 0; di < N_DIST; ++di) {
            ...
            for (int y = 0; y < H; y+=2) {
                for (int x = 0; x < W; x+=2) {
                    ...
                    for (int dy = (y - d + 1); dy <= ( y + d - 1); dy+=2) {
                        int dx = x - d;
                        if (dx >= 0 && dx < W && dy >= 0 && dy < H && (new Color(image.getRGB(dy, dx)).equals(c))) {
                            this.correlogramR[c.getRed()][di]++;
                            this.correlogramG[c.getGreen()][di]++;
    ...
    public double getDistance(Feature feature) {
        ApproximateColorCorrelogram approximateColorCorrelogram = (ApproximateColorCorrelogram) feature;
        float[][] otherCorrelogramR = approximateColorCorrelogram.getCorrelogramR();
        float[][] otherCorrelogramG = approximateColorCorrelogram.getCorrelogramG();
        float[][] otherCorrelogramB = approximateColorCorrelogram.getCorrelogramB();
        double distanceR = 0;
        double distanceG = 0;
        double distanceB = 0;
        double distance = 0;
